...
async def get_messages_by_room_uuid_user_joined(db: AsyncSession, room_uuid: str):
    query = select(Message).where(Message.room_uuid == room_uuid).options(joinedload(Message.user))
    result = await db.execute(query)
    return result.scalars().all()


async def create_message(db: AsyncSession, room_uuid: str, user_uuid: str, message: str, emotion: Emotion):
